...
class BaseTrainingRecipe:
    ...
    def add_args(self, model_args):
        ...
        if self.training_arguments.pretrained_model_path !="" and self.training_arguments.pretrained_model_path is not None:
            ...
            model_args['vq'].update(dict(pretrained_vq_path=os.path.join(self.training_arguments.pretrained_model_path, 'VQ_VAE')))
        return model_args
    ...
    def tune_type_setting(self, model):
        ...
        model = self._vq_tune_type_setting(model)
        return model    
    ...
    def _vision_tower_tune_type_setting(self, model):
        if not hasattr(model, 'vision_tower') or model.vision_tower is None:
            print("Warning: Model does not have vision_tower component, skipping vision tower tuning settings.")
            return model        
    ...
    def _connector_tune_type_setting(self, model):
        if not hasattr(model, 'connector') or model.connector is None:
            print("Warning: Model does not have connector component, skipping connector tuning settings.")
            return model
    ...
    def _vq_tune_type_setting(self, model):
        tune_type = self.training_arguments.train_VQ
        if tune_type=='full':
            for p in model.VQ.parameters():
                p.requires_grad = True
        elif tune_type=='codebook_frozen':
            for name, param in model.VQ.named_parameters():
                if name=='quantizer_text.quantizer.levels' or name=='quantizer_image.quantizer.levels' or name=='quantizer_single.quantizer.levels':
                    param.requires_grad = False
                    print("codebook frozened")
                else:
    ...
    def save(self, model, trainer):
        ...
        #sava VQ-VAE
        VQ_state_dict = get_state_maybe_zero_3(model.VQ.named_parameters(), [''], False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            VQ_output_dir = os.path.join(self.training_arguments.output_dir, 'VQ_VAE')
            os.makedirs(VQ_output_dir, exist_ok=True)
            VQ_output_path = os.path.join(self.training_arguments.output_dir, 'VQ_VAE/pytorch_model.bin')
            torch.save(VQ_state_dict, VQ_output_path)
    ...
    def load(self, model, model_args={}):
        if not ('lora' in self.training_arguments.pretrained_model_path and os.path.exists(os.path.join(self.training_arguments.pretrained_model_path, 'adapter_config.json'))): # loading model for non-lora/non-qlora pretraining
                    # ---- LLM ----
            if hasattr(model, "load_llm") and "llm" in model_args:
                print("[Load] Loading LLM...")
                model.load_llm(**model_args["llm"])

        # ---- Vision Tower ----
            if hasattr(model, "load_vision_tower") and "vision_tower" in model_args:
                print("[Load] Loading vision tower...")
                model.load_vision_tower(**model_args["vision_tower"])

        # ---- Connector ----
            if hasattr(model, "load_connector") and "connector" in model_args:
                print("[Load] Loading connector...")
                model.load_connector(**model_args["connector"])

        # ---- VQ ----
            if hasattr(model, "load_vq") and "vq" in model_args:
                print("[Load] Loading VQ module...")
                model.load_vq(**model_args["vq"])
        else:
