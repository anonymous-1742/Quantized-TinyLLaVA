...
from .VQ.vq import VQ_config,VQ
...
class TinyLlavaForConditionalGeneration(TinyLlavaPreTrainedModel):
    def __init__(self ,config: TinyLlavaConfig,VQ_type="single"):
        
        super().__init__(config)
        print("starting constrcting model")
        ...
        self.vq_config_text=VQ_config(token_dim=config.hidden_size,code_dim=config.hidden_size,
                                      discrete_size=config.discrete_size)
        self.vq_config_image=VQ_config(token_dim=config.hidden_size,code_dim=config.hidden_size,
                                       discrete_size=config.discrete_size)
        self.VQ=VQ(config.vq_type,self.vq_config_text,self.vq_config_image)
        self.VQ_type=config.vq_type
        print("VQ created")
        print("discrete size="+str(config.discrete_size))
        self.post_init()
    ...
    def get_activation(
        ...
        if inputs_embeds is None:
            image,text= self.prepare_activation(
                input_ids,
                position_ids,
                attention_mask,
                past_key_values,
            ...
            img_activation_np = image.detach().cpu().numpy()
            text_activation_np = text.detach().cpu().numpy()
            return(img_activation_np,text_activation_np)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            inputs_embeds,Loss=self.VQ.quantizer_single(inputs_embeds)
        ...
        Loss=Loss+vq_loss
            #print(self.VQ.quantizer.embedding[1,:])
        return output,Loss              
    ...
    def prepare_activation(
        ...
        quantized_text_embeds,vq_loss_text=self.VQ.quantizer_text(text_embeds,return_indice)
        return image_features, quantized_text_embeds
        
    def prepare_inputs_labels_for_multimodal(
        self, input_ids, position_ids, attention_mask, past_key_values, labels,
        images, image_sizes=None,return_indice=False
    ):
        vision_tower = self.vision_tower
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            print("model not executed")
        ...
        text_embeds=new_input_embeds[text_token_index].view(new_input_embeds.shape[0],-1,new_input_embeds.shape[2])
        quantized_text_embeds,vq_loss_text=self.VQ.quantizer_text(text_embeds,return_indice)
        new_input_embeds[text_token_index]=quantized_text_embeds.contiguous().view(-1,new_input_embeds.shape[2])
        vq_loss=vq_loss_img+vq_loss_text
        return input_ids, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels,vq_loss
