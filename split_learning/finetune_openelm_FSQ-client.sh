...
python tinyllava/train/tinyllava_client.py \
    ...
    --pretrained_model_path /nfs/turbo/umms-drjieliu1/usr/gjiajun/pretrain_client/tiny-llava-${LLM_VARIANT}-${VT_VARIANT}-${VERSION}-client-pretrain-${VQ_VERSION}-size-${VQ_SIZE}-${DATE} \
    --output_dir /nfs/turbo/umms-drjieliu1/usr/gjiajun/output_client/tiny-llava-${LLM_VARIANT}-${VT_VARIANT}-${VERSION}-client-${VQ_VERSION}-size-${VQ_SIZE}-${DATE} \
    --num_train_epochs 1 \
