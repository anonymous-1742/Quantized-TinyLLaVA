...
bash /scratch/drjieliu_root/drjieliu/gjiajun/TinyLLaVA_Factory/scripts/train/openelm/pretrain_act.sh "$DATA_PATH" "$IMAGE_PATH" "$LLM_VERSION" "$VT_VERSION" "$VT_VERSION2" "$CN_VERSION" "$VERSION" "$TRAIN_RECIPE" "$MODEL_MAX_LENGTH" "$VQ_VERSION" "$VQ_SIZE" "$DATE"
